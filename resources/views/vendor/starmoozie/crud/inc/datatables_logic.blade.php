@php
    ...
    $is_report = in_array($current_route, ['report-sale.index', 'report-expense.index']);
@endphp
    ...
    window.crud = {
        ...
        dataTableConfiguration: {
            ...
            footerCallback: function (row, data, start, end, display) {
                let is_report = "{{ $is_report }}";
                let currentRoute = "{{ $current_route }}";

                if (is_report) {
                    let api = this.api();
                    let rowNumber = currentRoute === 'report-sale.index' ? [5,6,7] : [4];

                    for (let index = rowNumber[0]; index <= rowNumber[rowNumber.length - 1]; index++) {
                        $(api.column(index).footer()).html(formatRupiah(sumRow(api, index)));
                    }
                }
            },
        }
